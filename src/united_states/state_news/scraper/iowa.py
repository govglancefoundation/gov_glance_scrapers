...
def main():
    url = 'https://governor.iowa.gov/newsroom' # url
    table = 'Iowa'   
    ...
    notification_title = 'Iowa State Updates'    # Notification title
    ...
    soup = BeautifulSoup(response.content, format).find('div', {'class':'view-content'})
    content = soup.find_all('div',{'class':'views-row'})
    ...
    for item in content: 
        ...
        if item.find('a') is not None:
            item_dict['url'] = 'https://governor.iowa.gov'+(item.find('a')['href'])
            item_dict['title'] = item.find('a').text
        if item.find('p') is not None:
            item_dict['description'] = item.find('p').text.replace('\n','').lstrip().rstrip()
        if item.find('time') is not None:
            item_dict['pubDate'] = (item.find('time')['datetime'].replace('\n','').lstrip().rstrip())
