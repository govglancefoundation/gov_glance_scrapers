...
def main():
    url = 'https://www.maine.gov/governor/mills/newsroom'           # url
    table = 'Maine'   
    ...
    notification_title = 'Maine State Updates'                    # Notification title
    ...
    for item in content: 
        ...
        if item.find('div') is not None:
            ...
            item_dict['description'] = item.find('div',{'class':summary}).text
        if item.find('a') is not None:
            item_dict['url'] = 'https://www.maine.gov'+(item.find('a')['href'])
