...
def main():
    url = 'https://governor.sc.gov/news'           # url
    ...
    notification_title = 'South Carolina State Updates'                    # Notification title
    ...
    regex = re.compile('.*views-col.*')

    # table_content = soup.find('div', {'class':'views-view-grid horizontal cols-4 clearfix'})
    ...
    for item in content: 
        ...
        if item.find('a') is not None:
            item_dict['link'] = 'https://governor.sc.gov/'+(item.find('a')['href']) # this is the pubDate
            item_dict['title'] = item.find('a').text
        if item.find('div') is not None:
            item_dict['pubDate'] = item.find('div',{'class':'views-field views-field-field-published-date'}).text
