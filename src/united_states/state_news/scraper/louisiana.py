...
def main():
    url = 'https://gov.louisiana.gov/index.cfm/newsroom/home' # url
    table = 'Louisiana'   
    ...
    notification_title = 'Louisiana State Updates'    # Notification title
    ...
    soup = BeautifulSoup(response.content, format).find('div', id='news-index')
    content = soup.find_all('div', {'class':'news-block'})
    ...
    for item in content: 
        ...
        if item.find('a') is not None:
            item_dict['link'] = 'https://gov.louisiana.gov'+(item.find('a',{'class':'read-more'})['href'])
        if item.find('h2') is not None:
            item_dict['title'] = item.find('h2').text
        if item.find('div') is not None:
            item_dict['pubDate'] = item.find('div',{'class':'date'}).text
            item_dict['description'] = item.find('div',{'class':'blurb'}).text
