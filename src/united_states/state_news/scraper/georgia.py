...
def main():
    url = 'https://gov.georgia.gov/press-releases' # url
    table = 'Georgia'   
    ...
    notification_title = 'Georgia State Updates'    # Notification title
    ...
    content = soup.find_all('div',{'class':"news-teaser"})
    ...
    for item in content: 
        ...
        if item.find('div') is not None:
            item_dict['pubDate'] = item.find("div", {'class': "global-teaser__description"}).text
        if item.find('a') is not None:
            item_dict['url'] ='https://gov.georgia.gov'+(item.find('a')['href'])
        if item.find('h2') is not None:
            item_dict['title'] = (item.find('h2').text.strip())
