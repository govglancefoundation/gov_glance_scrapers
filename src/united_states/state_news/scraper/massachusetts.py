...
def main():
    url = 'https://www.mass.gov/orgs/governor-maura-healey-and-lt-governor-kim-driscoll/news'           # url
    table = 'Massachusetts'   
    ...
    notification_title = 'Massachusetts State Updates'                    # Notification title
    ...
    table_content = soup.find('div', {'class':'ma__press-listing__items'})
    ...
    for item in content: 
        ...
        if item.find('a') is not None:
            item_dict['title'] = item.find('a').text # this is the pubDate
        if item.find('span') is not None:
            item_dict['pubDate'] = item.find('span',{'class':'ma__press-teaser__date'}).text.replace('\n', '').lstrip().rstrip()
        if item.find('p') is not None:
            item_dict['description'] = item.find('p').text
        if str(item.find('a')['href']).startswith(('http://', 'https://')):
            item_dict['link'] = (item.find('a')['href']) # this is the link
        else:
            item_dict['link'] = 'https://www.mass.gov/'+(item.find('a')['href'])
