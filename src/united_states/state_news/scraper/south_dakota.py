...
def main():
    url = 'https://governor.sd.gov/news/press-releases.aspx'           # url
    table = 'South Dakota'   
    ...
    notification_title = 'South Dakota State Updates'                    # Notification title
    ...
    content = soup.find_all('div',{'class':'newsitem'})
    ...
    for item in content: 
        ...
        if item.find('div') is not None:
            item_dict['pubDate'] = item.find('div', {'class':'newsdate'}).text
        if item.find('div',{'class':'newsarticle'}):
            item_dict['description'] = item.find('div',{'class':'newsarticle'}).text
