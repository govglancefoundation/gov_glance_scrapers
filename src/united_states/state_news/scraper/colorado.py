...
def main():
    url = 'https://www.colorado.gov/governor/news' # url
    table = 'Colorado'   
    ...
    notification_title = 'Colorado State Updates'    # Notification title
    ...
    content = soup.find_all('article', {'class': 'press-release--teaser'})
    # print(xml_string)
    data = []
    print(len(content))
    ...
    for item in content: 
        print(item)
        item_dict = {}
        if item.find('footer') is not None:
            item_dict['pubDate'] = (item.find('footer').text)
        if item.find('h2') is not None:
            item_dict['title'] = (item.find('h2').text)
        if item.find('a') is not None:
            item_dict['url'] = 'https://www.colorado.gov' + (item.find('a')['href'])
        if item.find('div') is not None:
            item_dict['description'] = (item.find('div').text)
