...
def main():
    url = 'https://portal.ct.gov/Office-of-the-Governor/News' # url
    table = 'Connecticut'   
    ...
    notification_title = 'Connecticut State Updates'    # Notification title
    ...
    content_table = soup.find('div', id = 'resultsPanel')
    content = content_table.find_all('li')
    ...
    for item in content: 
        ...
        if item.find('span') is not None:
            item_dict['pubDate'] = item.find('span',{'class':'date'}).text
        if item.find('a') is not None:
            item_dict['url'] ='https://portal.ct.gov'+(item.find('a')['href'])
