...
def main():
    url = 'https://www.nl.gob.mx/actualidad/boletines'          # url
    table = 'Nuevo Leon'   
    ...
    notification_title = 'Nuevo León State Updates'                    # Notification title
    ...
    for item in content: 
        item_dict = {}
        item_dict['link'] = 'https://www.nl.gob.mx' + item.find('a')['href']
        item_dict['title'] = item.find('h3').text
        item_dict['image'] = re.search(r'url\((.*?)\)',item.find('div', {'class':'block-news__image'})['style']).group(1)
