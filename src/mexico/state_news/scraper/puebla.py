...
def main():
    url = 'https://www.puebla.gob.mx/index.php/noticias'          # url
    table = 'Puebla'   
    ...
    notification_title = 'Puebla State Updates'                    # Notification title
    ...
    content = soup.find_all('div', {'class':'itemcat'})
    data = []
    ...
    for item in content[:10]: 
        item_dict = {}
        item_dict['link'] = 'https://www.puebla.gob.mx' + item.find('a')['href']
        item_dict['title'] = item.find('div', {'class':'itemcat-title'}).text.lstrip().rstrip()
        item_dict['image'] = 'https://www.puebla.gob.mx' + item.find('img')['src']
        item_dict['pubDate'] = item.find('div', {'class':'itemcat-created'}).text.lstrip().rstrip()
