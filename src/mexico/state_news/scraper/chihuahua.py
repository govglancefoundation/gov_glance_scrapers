...
def main():
    url = 'https://chihuahua.gob.mx/prensa?zona_prensa&page=0'          # url
    table = 'Chihuahua'   
    ...
    notification_title = 'Chihuahua State Updates'                    # Notification title
    ...
    content = soup.find_all('div', {'class': 'noticia'})
    data = []
    ...
    for item in content: 
        ...
        item_dict['pubDate'] = ' '.join([i.text for i in item.find_all('time')])
        item_dict['link'] = 'https://chihuahua.gob.mx' +item.find('a')['href']
        item_dict['image'] = 'https://chihuahua.gob.mx' + item.find('img')['src']
        item_dict['title'] = item.find('p', {'class':'fs-md'}).text
        if item.find('div', {'class':'views-field-field-bullet'}).text:
            item_dict['description'] = item.find('div', {'class':'views-field-field-bullet'}).text
