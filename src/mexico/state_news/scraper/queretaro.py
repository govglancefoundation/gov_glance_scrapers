...
def main():
    url = 'https://portal.queretaro.gob.mx/prensa/default.aspx'          # url
    table = 'Queretaro'   
    ...
    notification_title = 'Queretaro State Updates'                    # Notification title
    ...
    content = soup.find_all('div', {'class':'col-md-8'})
    data = []
    ...
    for item in content[1:]: 
        item_dict = {}
        item_dict['link'] = 'https://portal.queretaro.gob.mx/prensa/' + item.find('a')['href']
        item_dict['title'] = item.find('h3').text
        item_dict['pubDate'] = item.find('h4').text
        if item.find('p').text:
            if len(item.find('p').text) > 1:
