...
def main():
    url = 'https://www.col.gob.mx/Portal/listado_noticias'          # url
    table = 'Colima'   
    ...
    notification_title = 'Colima State Updates'                    # Notification title
    ...
    content = soup.find_all('div', {'class':'noti-section'})
    data = []
    ...
    for item in content: 
        ...
        item_dict['title'] = item.find('a').find_next_sibling().find('a').text.rstrip().lstrip()
        item_dict['image'] = item.find('img')['src']
        item_dict['pubDate'] = item.find('div', {'class':'col-2'}).text.replace('\n',' ')
